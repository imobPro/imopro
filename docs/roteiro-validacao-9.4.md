...
> **Resumo da validação (2026-05-23, re-run completo pós-Sprint 9.6, via Playwright MCP)**
> Re-execução integral do roteiro após Sprint 9.6 (commits 9f96f4b + 0b68c1b) para garantir que nenhuma das mudanças (auth do webhook por instanceId, banner de desconexão, remoção do ZAPI_CLIENT_TOKEN) causou regressão. **1 bug novo encontrado (#7)** — todos os outros 6 anteriores continuam corrigidos. Cobertura por seção: 1 ✅ (1.1–1.9 limpas, mobile 375 sem overflow horizontal; nota: tabelas em /privacidade e /termos vão como parágrafo literal com `|` por falta de remark-gfm no react-markdown — preexistente, não bloqueia 1.5/1.6); 2 ✅ (2.1–2.4); 3 ✅ (3.1–3.6; observação não-bloqueante: Step 3 exibe telefone literal "abc" no resumo em vez do valor sanitizado, mas o envio usa `digitsOnly` então não polui o banco); 4 ✅ (4.1–4.7 todos confirmados via Supabase REST — userId 2cdf2a36..., tenantId e218fd24..., agentId 5e76b8f2..., subscription trial sem datas); 5 ✅ (5.1 toast PT + wizard volta pro Step 1 com dados preservados); 6 ✅ obsoleta confirmada (cadastro → /verificar-email → /conectar-whatsapp instantâneo); 7 parcial (7.1, 7.2 ✅ tela de erro com "Tentar novamente"; 7.3–7.6 exigem tokens Z-API reais — fora do escopo do dev local); 8 ✅ (8.1–8.5 com banner neutro "Trial: 7 dias e 50 mensagens restantes" + link "Ver assinatura"); 9 ✅ (9.1 âmbar 2 dias, 9.2 vermelho expired, 9.3 página "Trial encerrado" + badge danger, 9.5 botões Falar/Enviar com SUPPORT vars setadas, 9.6 plano ativo banner some, 9.7 revert mostra neutro "Conecte seu WhatsApp"; 9.4 fallback sem SUPPORT vars não testado pra evitar reinício do Next, cobertura via código `subscription.toBannerVariant`); 10 ✅ (10.1 /precos→/inbox, 10.2 /privacidade logado OK, 10.3 sem cookies /inbox→/login, 10.4 backend parado TrialBanner some painel acessível, 10.5 dark mode aplica em tudo); 11 pulado (SENTRY_DSN vazio por design em dev). **Delta 9.6 validado**: banner danger "Seu WhatsApp desconectou. Reconecte..." renderiza com `tenants.zapi_status='disconnected'`; `POST /webhook/whatsapp` com `instanceId` desconhecido devolve `200 {"action":"ignored_unknown_instance"}` sem precisar de header `Client-Token`; boot do backend confirma `ZAPI_CLIENT_TOKEN` ausente sem warning (env validation atualizada). Estado pós-validação: backend rodando em 3001, Redis ainda **não instalado nesta máquina** (gera ruído ECONNREFUSED constante nos logs do backend mas não bloqueia HTTP/Supabase — worker BullMQ não funciona, irrelevante pro escopo deste roteiro); novo user `arthur.cg12+teste7@gmail.com` (tenant e218fd24-dbb0-4d3b-b4ac-49f8289f5890, agentId 5e76b8f2-5a1b-43e3-81ef-b05b45aabdb9) revertido pra `status='trial', trial_started_at=NULL, zapi_status='not_provisioned'`; users teste1..6 anteriores permanecem; `.env.local` do frontend mantém `NEXT_PUBLIC_SUPPORT_WHATSAPP` e `NEXT_PUBLIC_SUPPORT_EMAIL` setados.
...
### Bug #7 — Hydration mismatch no `<input>` de busca da inbox (FieldControl/base-ui)
**Encontrado em:** Re-run 2026-05-23. Não estava no resumo de 2026-05-22 — provavelmente surgiu depois com bump do `@base-ui-components/react` ou interação com Next 16.
**Onde:** `frontend/src/app/(app)/inbox/_components/conversations-list.tsx` (ou wherever the search `<Input type="search">` lives), via `FieldControl` do base-ui dentro do componente `Input`.
**Esperado:** Console limpo após o fix do Bug #6.
**Aconteceu:** Toda navegação para `/inbox` loga no console: "A tree hydrated but some attributes of the server rendered HTML didn't match the client properties". Diff aponta exclusivamente para o `<input id="base-ui-_R_55kneaitmlb_" type="search" ...>` — no server o `style` é ausente, no client o React aplica `style={{caret-color:"transparent"}}`. Esse atributo vem do `FieldControl` quando ele detecta foco/teclado virtual; aplicação só no client gera divergência.
**Repro:** abrir DevTools → Console, logar e visitar `/inbox`. 1 erro de hydration por carregamento. Não quebra a funcionalidade (a UI continua respondendo, busca funciona), mas polui o console e o overlay do Next mostra "1 Issue".
**Causa provável:** `caret-color: transparent` é um efeito do `FieldControl` quando há `aria-disabled` ou `data-readonly` calculados em runtime após hidratar (`useEffect`). SSR não roda esse efeito → estilo ausente. Análogo ao padrão do Bug #6 (ThemeToggle), mas em outro componente.
**Sugestão de fix:** opção mais simples — `suppressHydrationWarning` no `<input>` do componente `Input` quando `type="search"` (cirúrgico). Opção mais correta — investigar se `FieldControl` aceita uma prop pra desligar o caret-color no SSR, ou wrapping em `useEffect`+`mounted` flag como foi feito no `ThemeToggle`. Avaliar antes de propagar `suppressHydrationWarning` por aí (cada uso engole avisos legítimos futuros). **Não bloqueia o piloto**, mas é dívida que vai poluir o Sentry quando subir a flag.
