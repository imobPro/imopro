...
> Migrations 011, 012 e 013 já aplicadas no Supabase.
...
| ~~**Client-token por instância (Z-API)**~~ ✅ | Resolvido em 2026-05-22 (Sprint 9.6): `/webhook/whatsapp` autenticava por posse do `instanceId`. **Substituído em 2026-08-12** (Etapa 2 da auditoria, achados #1 e #2) — o modelo de "instanceId é segredo" era frágil (id aparece em cleartext no DB, em painéis e em URLs). Novo esquema: coluna `tenants.webhook_secret` (migration 013, gerada com `gen_random_bytes(32)`), rotas `/webhook/whatsapp/:secret` e `/webhook/zapi-status/:secret` com middleware `requireWebhookSecret` + rate limit por secret + checagem defense-in-depth de spoofing de `instanceId`. Ver `lessons.md` 2026-08-12. |
| **Reapontar callbacks Z-API para URL com `:secret`** ⏸️ | Migration 013 aplicada (2026-08-12), backend com rotas novas commitado. **Bloqueado por licença Z-API** — precisa chamar `PUT /instances/{id}/token/{token}/update-webhook-received` (e `-connected`/`-disconnected`) com header `Client-Token`. Rotas legacy (`/webhook/whatsapp` e `/webhook/zapi-status` sem `:secret`) continuam ativas para não perder mensagem durante a janela — remover em commit específico após confirmar que a Z-API está batendo no path novo (24-48h de log sem o warning `[Webhook] Rota legacy sem :secret`). **Rotacionar o `webhook_secret`** logo após a rotação porque o valor apareceu no chat durante a operação. |
